...
float4 PS( VS_OUTPUT input ) : SV_Target
{
	...
	ComputeSpotLight(Mater, SpLight1, input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);
	...
	float FogPercent = saturate((distToEye - FgData.FogStart) / FgData.FogRange);  // See https://msdn.microsoft.com/en-us/library/windows/desktop/bb509645(v=vs.85).aspx

	litColor = mainTexture.Sample(aSampler, input.Tex) * litColor;
	litColor = lerp(litColor, FgData.FogColor, FogPercent);
